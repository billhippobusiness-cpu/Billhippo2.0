...
import { Save, Building2, MapPin, Mail, Phone, ShieldCheck, CreditCard, Info, Zap } from 'lucide-react';
...
const ProfileSettings: React.FC = () => {
  const [profile, setProfile] = useState<BusinessProfile>({
    name: 'Bharat Electronics & Solutions',
    tagline: 'Empowering Digital India',
    gstin: '27AABCB1234A1Z1',
    address: '402, Business Hub, MG Road',
    city: 'Mumbai',
    state: 'Maharashtra',
    pincode: '400001',
    ...
    upiId: 'bharat.solutions@hdfcbank',
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 pb-20">
      <div className="flex justify-between items-end mb-2">
        ...
        >
          {isSaving ? 'Synchronizing...' : <><Save size={20} /> Save Configuration</>}
        </button>
      </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-10 premium-shadow border border-slate-50 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-poppins">
              <Input label="Legal Name" value={profile.name} onChange={v => setProfile({...profile, name: v})} />
              <Input label="Email Address" value={profile.email} onChange={v => setProfile({...profile, email: v})} />
              <Input label="GSTIN Number" value={profile.gstin} onChange={v => setProfile({...profile, gstin: v})} />
              <Input label="PAN Number" value={profile.pan} onChange={v => setProfile({...profile, pan: v})} />
              <Input label="Contact Number" value={profile.phone} onChange={v => setProfile({...profile, phone: v})} />
              <Input label="Business Tagline" value={profile.tagline || ''} onChange={v => setProfile({...profile, tagline: v})} />
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-10 premium-shadow border border-slate-50 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-poppins">
              ...
              <Input label="Account Number" value={profile.accountNumber || ''} onChange={v => setProfile({...profile, accountNumber: v})} />
              <Input label="IFSC Code" value={profile.ifscCode || ''} onChange={v => setProfile({...profile, ifscCode: v})} />
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-10 premium-shadow border border-slate-50 space-y-8">
            ...
            <div className="space-y-4 font-poppins">
              <Input label="Street Address" value={profile.address} onChange={v => setProfile({...profile, address: v})} />
